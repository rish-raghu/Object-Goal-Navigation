    ...
    for step in range(args.num_training_frames // args.num_processes + 1):
        ...
        for e in range(num_scenes):
            ...
            if (local_map[e, cn, :, :] > args.sem_goal_thr).any():
                if not wait_env[e] and not saved[e]:
                    np.save('{}/localmap_{}_{}'.format(dump_dir, infos[e]["scene"], infos[e]["episode_id"]), local_map[e].cpu().numpy())
                    savedata = {}
                    savedata["local_pose"] = [float(x) for x in local_pose[e].cpu().tolist()]
                    savedata["origin"] = list([float(x) for x in origins[e]])
                    savedata["goal_cat_id"] = int(infos[e]['goal_cat_id'])
                    savedata["sim_pos"] = list([float(x) for x in infos[e]["sim_pos"]])
                    savedata["sim_rot"] = list([float(x) for x in infos[e]["sim_rot"]])
                    savedata["scene"] = infos[e]["scene"]
                    savedata["floor_idx"] = int(infos[e]["floor_idx"])
                    savedata["goal_name"] = infos[e]["goal_name"]
                    savedata["episode_id"] = int(infos[e]["episode_id"])
        ...
        if args.eval and len(savedatas) % args.episode_save_interval == 0:
            with open('{}/{}_episode_data.json'.format(
                dump_dir, args.split), 'w') as f:
                    json.dump(savedatas, f)
