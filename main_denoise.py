...
from model import RL_Policy, Semantic_Mapping, Semantic_Denoising
...
def main():
    ...
    if args.train_denoise:
        with bz2.BZ2File('data/datasets/objectnav/gibson/v1.1/{}/{}_info.pbz2'.format(args.split, args.split), 'rb') as f:
    ...
    def gt_to_global_map(mp, global_start_rot, gt_start_pos):
    ...
    denoise_net = Semantic_Denoising().to(device)

    if args.load_denoise != "0":
        print("Loading denoising model {}".format(args.load_denoise))
        state_dict = torch.load(args.load_denoise,
                                map_location=lambda storage, loc: storage)
        denoise_net.load_state_dict(state_dict)
    if args.train_denoise:
        trainable_params = [param for param in denoise_net.parameters() if param.requires_grad]
    ...
    for step in range(args.num_training_frames // args.num_processes + 1):
        ...
        if args.train_global and g_step % args.num_global_steps == args.num_global_steps - 1 \
        ...
        # Semantic denoising training
        if args.train_denoise and l_step == args.num_local_steps - 1:
            optimizer.zero_grad()
            cleaned_full_map = denoise_net(full_map[:, 4:-1, :, :])
            explored_gt_map = torch.clone(full_gt_map).detach()
            for e in range(num_scenes):
                unexplored = (full_map[e, 1, :, :] == 0)
                explored_gt_map[e, :, unexplored] = 0
            map_loss = mse_loss(cleaned_full_map, explored_gt_map)
            map_losses.append(100*map_loss.cpu())
            map_loss.backward()
            optimizer.step()
        ...
        if step % args.log_interval == 0:
            ...
            if len(map_losses) > 0 and args.train_denoise:
                log += "   Map loss: {:.3f}".format(np.mean(map_losses))
        ...
        if (step * num_scenes) % args.save_periodic < \
            ...
            if args.train_denoise:
                torch.save(denoise_net.state_dict(),
                           os.path.join(dump_dir,
                                        "denoise_periodic_{}.pth".format(total_steps)))
        # ------------------------------------------------------------------
